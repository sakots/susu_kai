...
$URL = $URL = empty($_SERVER['HTTPS']) ? 'http://' : 'https://'.$_SERVER['HTTP_HOST'].$match[1];
$SCRIPT = $match[2];
$BASEURL = "$URL/$bbs/";
#設定ファイルを読む
...
<div style="background-color:<?=$SETTING['BBS_THREAD_COLOR']?>; padding: 10px;">
<? readfile('headad.txt'); ?>
<a href="./">掲示板に戻る</a>
<?php
...
<tr>
<td nowrap align="right">名前:</td><td><input name="FROM" style="width:50%;"></td>
</tr>
<tr>
<td nowrap align="right">E-mail:</td><td><input name="mail" style="width:50%;"></td>
</tr>
<tr>
<td nowrap align="right" valign="top">内容:</td><td><textarea style="width:90%; height:80px;" wrap="soft" name="MESSAGE"></textarea></td>
</tr>
<tr>
<td nowrap align="right" valign="top">画像:</td><td><input type="file" name="file" style="width:50%;"></td>
</tr>
<tr>
<td></td><td><input type="hidden" name="bbs" value="<?=$bbs?>">
<input type="hidden" name="subject" value="<?=$sbj?>">
<input type="hidden" name="key" value="<?=$key?>">
<input type="hidden" name="time" value="<?=$NOW?>">
<input type="submit" value="書き込む" name="submit"></td>
</tr>
</table>
</form>
<?php
...
<dl>
<dt>1 名前:<font color="green"><b><?=VERSION?></b></font>投稿日:2001/04/12(木) 15:11</dt>
<dd><?=$topic?></dd>
</dl>
<hr>
<?
	if (is_file("../$bbs/kako/$key.html")) {
		?>
<p>過去ログ倉庫で、<a target="_self" href="<?="$URL/$bbs/kako/$key.html"?>">スレッド<?=$key?>.html</a> を発見しました。</p>
<?php
	}
	else {
		?>
<p><a target="_self" href="<?="$URL/$bbs/kako/"?>">過去ログ倉庫</a>にもありませんでした。問い合わせても見つかる可能性はほとんどありません。</p>
<?php
	}
	echo "</body>\n</html>";
	if (GZ_FLAG) ob_end_flush();
