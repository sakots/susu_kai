...
function MakeWorkFile($bbs, $key, $outdat="") {
	...
	if (is_writable($dattemp) or !is_file($dattemp)) {
		...
		if ($lognum >= THREAD_RES) {
			if (!$stop) {
				# 全角数字に変更
				$maxnum = mb_convert_kana(THREAD_RES, "asKV", "UTF-8");
				$maxplus = mb_convert_kana(++$lognum, "asKV", "UTF-8");
				$maxmsg = "このスレッドは${maxnum}を超えました。 <br> もう書けないので、新しいスレッドを立ててください。";
				if (THREAD_MAX_MSG) {
	...
	$logall = '<table border="0" cellspacing="7" cellpadding="3" width="95%" bgcolor="'.$SETTING['BBS_THREAD_COLOR'].'" align="center"><tr><td><dl><a name="$ANCOR"></a><div align="right"><a href="#menu">■</a><a href="#$FRONT">▲</a><a href="#$NEXT">▼</a></div><b>【$ANCOR:'.$lognum.'】<font size="5" color="'.$SETTING['BBS_SUBJECT_COLOR']."\">$subject</font></b>\n";
	#１つ目のリンクを作成
	...
	#１つ目の要素を吐き出す
	$logall .= "<dt>1 名前：$mailto $date</dt><dd>$message</dd><br><br><br>\n";
	#残りのログを表示する
	foreach ($logopen as $tmp){
		...
		#要素を吐き出す
		$logall .= "<dt>$topnum 名前：$mailto ：$date</dt><dd>";
		// 0thelloスレッドは全部表示
		if ($GLOBALS['vip'][8]) $logall .= $message."</dd>";
		else {
			$messx = explode ("<br>", $message."</dd>");
