...
if ($_POST['subject']) {
	...
	// トリップ付け忘れならデータファイル削除
		...
		DispError("ERROR!", "VIP-0THELLO:トリップをつけてください");
	}
}
	...
	if ($black) {
		...
		if ($trip == $now) {
			...
			if (preg_match("/(.*)!0thello!x(\d+)!y(\d+)<\/b>◆/", $_POST['FROM'], $match)) {
				if ($match[2] < 1 or $match[2] > 9) DispError("ERROR!", "VIP-0THELLO: X軸を指定してください ");
				if ($match[3] < 1 or $match[3] > 9) DispError("ERROR!", "VIP-0THELLO: Y軸を指定してください ");
				...
				$x = $match[2] - 1;
				$y = $match[3] - 1;
				if ($game_board[$y][$x] != "　") DispError ("ERROR!","VIP-0THELLO：そこに駒を置くことはできません");
				...
				$reverse += reverse($game_board, $tmp_board, $x, $y, 1, 0, $p1, $p2);
				$reverse += reverse($game_board, $tmp_board, $x, $y, 1, 1, $p1, $p2);
				if ($reverse == 0) DispError ("ERROR!","VIP-0THELLO：そこに駒を置くことはできません");
				$game_board[$y][$x] = $p1;
				fseek($fp, 0, SEEK_SET);
				fwrite($fp, $white."\n".$black."\n".$game_count.",".$next."\n");
				for ($i = 0; $i < 8; $i++) {
