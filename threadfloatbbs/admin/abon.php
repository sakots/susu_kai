...
if (!is_dir("../$_REQUEST[bbs]")) disperror("ERROR!", "そんな板orスレッドないです。");
#====================================================
#　初期情報の取得（設定ファイル）
#====================================================
#設定ファイルを読む
...
else disperror("ERROR!","ERROR:ユーザー設定が消失しています!");
...
if (isset($_POST['mode']) and $_POST['mode'] == "res_del" and isset($_POST['del'])) {
	if (!is_file("../$_POST[bbs]/dat/$_POST[key].dat")) disperror("ERROR!", "そんな板orスレッドないです。");
	$datafile = "../$_POST[bbs]/dat/$_POST[key].dat";
	if (!is_writable($datafile)) disperror("ERROR!", "このスレッドには書き込めません!");
	$temp = file($datafile);
	$num = count($temp);
...
if(isset($_REQUEST['mode']) and ($_REQUEST['mode'] == "view" or $_REQUEST['mode'] == "res_del")) {
	if (!is_file("../$_REQUEST[bbs]/dat/$_REQUEST[key].dat")) disperror("ERROR!", "そんな板orスレッドないです。");
...
elseif (isset($_POST['mode']) and $_POST['mode'] == "stop") {
	...
	if (!is_writable($dattemp)) disperror("ERROR!", "すでに書きこみ出来ません。");
	else {
		$fp = fopen($dattemp, "a");
